...
const { pool } = require("../db"); // Assuming a PostgreSQL connection setup
...
router.post("/", async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Membership created", membership: result.rows[0] });
    } catch (error) {
...
router.put("/:id", async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Membership updated", membership: result.rows[0] });
    } catch (error) {
